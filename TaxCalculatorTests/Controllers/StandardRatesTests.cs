...
namespace TaxCalcuator.Tests
{
    ...
    public class ProgressiveTaxCalculatorTests
    {
        ...
        public ProgressiveTaxCalculatorTests()
        {
            var dbSettings = new DatabaseSettings() { DBConnectionString = "Server=(LocalDB)\\MSSQLLocalDB;AttachDbFilename=|DataDirectory|\\Data\\TaxBandsSample.mdf;Integrated Security=True;" };
            var options = Options.Create(dbSettings);

            ITaxBandsRepository taxRateRepository = new TaxBandsRepository(options);

            // Instantiate a real tax calculator to use for these tests
            _taxCalculator = new ProgressiveTaxCalculator(taxRateRepository);
        }
